...
import { DUMMY_ITEM_HEIGHT, HEADER_HEIGHT, ICON_HEIGHT, SCREEN_HEIGHT, TOP_PADDING, } from "./constants/AppConstant";
export default function Container({ title, icon, subtitle, children, onClose, bgColor, customIcon, tintColor, actions }) {
    ...
    return (<View style={[
      ...
      <Animated.ScrollView onScroll={handleScroll} scrollEventThrottle={16} style={{ flex: 1 }} scrollIndicatorInsets={{ top: HEADER_HEIGHT, bottom: bottom + 10 }} contentContainerStyle={[styles.scrollViewContent, { paddingBottom: bottom + 50 }]}>
        {customIcon &&
            <Image source={customIcon.source} tintColor={customIcon.tintColor ? correctHex(customIcon.tintColor) : theme.colors.text} style={[styles.icon, { height: 1.2 * ICON_HEIGHT, width: 1.2 * ICON_HEIGHT }]}/>}
        {!customIcon &&
